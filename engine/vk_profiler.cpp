...
namespace vkutil 
{
    ...
    void VulkanProfiler::init(VkDevice device, float timestampPeriod, int perFramePoolSizes)
	{
		...
		_device = device;
		_currentFrame = 0;
        _poolSize = perFramePoolSizes;

        VkQueryPoolCreateInfo createInfo = {};
        createInfo.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO;
        createInfo.queryType = VK_QUERY_TYPE_TIMESTAMP;
        createInfo.queryCount = _poolSize;

        for (size_t i = 0; i < QUERY_FRAME_OVERLAP; i++)
		{
            vkCreateQueryPool(_device, &createInfo, nullptr, &_queryFrames[i]._timerPool);
	...
	void VulkanProfiler::grab_queries(VkCommandBuffer cmd)
	{
		...
		vkCmdResetQueryPool(cmd, _queryFrames[_currentFrame]._timerPool, 0, _poolSize);
		_queryFrames[_currentFrame]._timerLast = 0;
		_queryFrames[_currentFrame]._frameTimers.clear();

		vkCmdResetQueryPool(cmd, _queryFrames[_currentFrame]._statPool, 0, _poolSize);
	...
	void VulkanProfiler::cleanup()
	{
        for (size_t i = 0; i < QUERY_FRAME_OVERLAP; i++)
		{
			...
			vkDestroyQueryPool(_device, _queryFrames[i]._statPool, nullptr);
		}
	}
	...
	uint32_t VulkanProfiler::get_stat_id()
	{
		...
		_queryFrames[_currentFrame]._statLast++;
		return q;
	}
}